...
<template>
    <div class="event_page">
        ...
        ></Search>
        <section class="search_tip">-- 当前设备检索范围：{{ device_range }} --</section>
        <DropDownMenu
                class="DropDownMenu"
...
export default {
    ...
    methods: {
        ...
        choose_item(my_arguments, index) {
            ...
            this.enter_cow_page(this.muti_list_items[muti_item_index]['data'][item_index].cow_id)
        },
        async get_event() {
            let event_type = this.options_set[0][this.pitch_on_options_index[0]]
            let date_range = this.options_set[1][this.pitch_on_options_index[1]]
            switch (date_range) {
                case '一天内':
                    date_range = 1
                    break
            ...
            await EVENT.get_event(event_type, date_range, this.device_range, this.region).then((res) => {
        ...
        enter_cow_page(cow_id) {
            // this.is_select = false
            this.$router.push({path: '/CowInfo', query: {cow_info: cow_id}})
        },
        async search_value_change(value) {
            this.isLoading = true;
            await DEVICE.fuzzy_search_imeis(value, this.region).then((res) => {
                this.fuzzy_items = res
                this.isLoading = false;
            })
        },
        choose_fuzzy_block(item) {
            this.device_range = `${item['设备号']}号设备`
            this.get_event()
        },
        clear_search_value() {
            this.device_range = '所有设备'
            this.get_event()
        }
    },
}
